package edu.cmu.cs.cmulab.elan;
...
/**
 * Generic ELAN recognizer that calls APIs provided by the
 * CMU Linguistic Annotation Backend server
 * Code adapted from ELAN's DemoRecognizer
 */
public class MultiRecognizer implements Recognizer { 
	private static String EVEN_LABEL = "E";
	private static String UNEVEN_LABEL = "U";
	private RecognizerHost host;
	private MultiRecognizerPanel controlPanel;
	private boolean keepRunning;
	private String currentMediaFilePath;
	float duration = 12000f;
	private String name = "CMU Linguistic Annotation Backend";
	...
	public JPanel getControlPanel() {
		if (controlPanel == null) {
			controlPanel = new MultiRecognizerPanel(host.getSelectionPanel(null), host);
		}
		return controlPanel;
	}
	...
	public boolean setMedia(List<String> mediaFilePaths) {
		((MultiRecognizerPanel)getControlPanel()).updateMediaFiles(mediaFilePaths);
		return true;
	}
	 ...
	 * as defined by the slider on the MultiRecognizerPanel
	 *
	 */
